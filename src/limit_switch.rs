...
/// 1: Y-Limit
/// 2: Z-Limit
static LIMIT_SWITCH_STATES: Mutex<RefCell<[bool; 3]>> = Mutex::new(
    RefCell::new(
    [true, true, true]
    )
);
...
impl LimitSwitch {
    pub fn new(pin: Mutex<Cell<Pin<Input<PullUp>, Dynamic>>>, switch_index: usize) -> Self {
        Self {
            switch_index
        }
    }
    ...
    pub async fn wait_for(&mut self, desired_state: bool) {
        poll_fn(|cx| {
            if interrupt::free(|cs| {
                ...
                interrupt::free(|cs| {
                    LIMIT_SWITCH_TASKS[self.switch_index].borrow(cs).replace(cx.waker().task())
                });
                Poll::Pending
            }
        }).await
...
fn PCINT2() {
    ...
    interrupt::free(|cs| {
        ...
        let switches = [limit_x_pin.downgrade(), limit_y_pin.downgrade(), limit_z_pin.downgrade()];
        ...
        for (index ,switch_state ) in switch_states.iter().enumerate() {
            if switches.get(index).unwrap().is_high() != *switch_state {
                let limit_switch_task = LIMIT_SWITCH_TASKS.get(index).unwrap().borrow(cs).replace(0xFFFF);
